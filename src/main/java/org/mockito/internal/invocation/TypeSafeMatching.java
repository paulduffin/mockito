...
public class TypeSafeMatching implements ArgumentMatcherAction {
    ...
    private static boolean isCompatible(ArgumentMatcher<?> argumentMatcher, Object argument) {
        ...
        Class<?> expectedArgumentType = getArgumentType(argumentMatcher, "matches");
     ...
     */
    public static <T> Class<T> getArgumentType(ArgumentMatcher<T> argumentMatcher, String methodName) {
        Method[] methods = argumentMatcher.getClass().getMethods();

        for (Method method : methods) {
            if (isMatchesMethod(method, methodName)) {
                return (Class<T>) method.getParameterTypes()[0];
            }
        }
        throw new NoSuchMethodError("Method '" + methodName + "(T)' not found in ArgumentMatcher: "
     ...
     */
    private static boolean isMatchesMethod(Method method, String methodName) {
